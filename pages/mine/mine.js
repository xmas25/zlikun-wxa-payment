...
Page({
  ...
  onPayment: function(e) {
    ...
    // 统一下单
    wx.request({
      url: getApiUrl('/pay/unified-order'),
      data: {
        token: app.globals.loginToken
      },
      method: "POST",
      header: {
        'content-type': 'application/x-www-form-urlencoded'
      ...
      success: res => {
        console.log(res)
        // 调起支付（这里仅做测试用，调起支付并不必要在统一下单的成功回调里执行，这里仅为了简单避免异步问题）
        // https://developers.weixin.qq.com/miniprogram/dev/api/open-api/payment/wx.requestPayment.html
        wx.requestPayment({
...
function getApiUrl(path) {
  return 'http://wx.zlikun.com' + path
}